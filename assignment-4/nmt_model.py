...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        indices = torch.tensor(self.vocab.src.words2indices(target_padded.tolist()))
        Y = self.model_embeddings.target(indices)
        for t in torch.split(Y, 1):
            Y_t = torch.squeeze(t, 0)
